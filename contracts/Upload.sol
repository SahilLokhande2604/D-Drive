...
contract Upload {
  ...
  event AccessGranted(address indexed owner, address indexed user, string file);
  event AccessRevoked(address indexed owner, address indexed user, string file);
  ...
  function getaccessibleaccounts(string memory file)external view returns (address[] memory) {
      return imageAccessList[file];
  }
  function getAccessibleFiles(address owner, address user) external view returns (string[] memory) {
  ...
  function removeAddressFromImageAccessList(string memory file, address user) internal {
